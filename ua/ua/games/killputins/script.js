...
questionmark.add_event_listener( "click" , function() {
    toast.makeToast("Вбий усіх путінів! Підказки: Збирай монети, щоб поповнювати здоров'я та ману. Щоб купити здоров'я та ману клікні на значок монетки");
});
...
var update_stats = function() {
    ...
    if(health < 0) {
        ...
        update_stats();
        toast.makeToast("Ви програли :с", 1000);
        setTimeout(function() {
            load_level(level);
        },50);
    }
    ...
    var setup = function() {
        ...
        var change_func = function(e) {
            if(is_targeted(e.target, change_div)) {
                ...
                if(color === "rgb(0, 0, 0)") {
                    pause();
                    toast.makeToast("Перетягніть кнопки управління, щоб змінити їхнє положення");
...
var handle_enemy_collision = function() {
    for(var i in enemies) {
        ...
        !mage.invincible && !paused) {
            toast.makeToast("Вам завдано шкоди від хуйла! Негайно розберіться із цим!"); // vvv
            health--;
            update_stats();
...
var load_level = function(id) {
    ...
    if(lvl && (coin_counter >= lvl.cost || lvl.unlocked)) {
        ...
        toast.makeToast("Вам потрібно "+lvl.cost
        +" монеток, щоб попасти в цей рівень "+id+"!");
    } else {
        toast.makeToast("Цей рівень "+id+" знаходиться у розробці");
    }
};
