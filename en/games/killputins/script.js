...
questionmark.add_event_listener( "click" , function() {
    toast.makeToast("Kill all putins! Hints: Collect coins to replenish health and mana. To buy health and mana, click on the coin icon");
});
    ...
    var setup = function() {
        ...
        var change_func = function(e) {
            if(is_targeted(e.target, change_div)) {
                ...
                if(color === "rgb(0, 0, 0)") {
                    pause();
                    toast.makeToast("Drag control buttons to change their position");
...
var handle_enemy_collision = function() {
    for(var i in enemies) {
        ...
        !mage.invincible && !paused) {
            toast.makeToast("You've been hit by a dick! Deal with this immediately!"); // vvv
            health--;
            update_stats();
...
var load_level = function(id) {
    ...
    if(lvl && (coin_counter >= lvl.cost || lvl.unlocked)) {
        ...
        +" coins, to get to this level "+id+"!");
    } else {
        toast.makeToast("This level "+id+" is under development");
    }
};
