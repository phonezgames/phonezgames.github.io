...
var link = "https://phonezgames.xyz/img/mage-sprite-event.png";
...
questionmark.add_event_listener( "click" , function() {
    toast.makeToast("Убей всех путинов! Подсказки: Собирай монетки, чтобы пополнять здоровье и ману. Чтобы купить здоровье и ману кликни на значёк монетки");
});
...
var update_stats = function() {
    ...
    if(health < 0) {
        ...
        update_stats();
        toast.makeToast("Вы проиграли :с", 1000);
        setTimeout(function() {
            load_level(level);
        },50);
    }
    ...
    var setup = function() {
        ...
        var change_func = function(e) {
            if(is_targeted(e.target, change_div)) {
                ...
                if(color === "rgb(0, 0, 0)") {
                    pause();
                    toast.makeToast("Перетащите кнопки управления, чтобы изменить их положение");
...
var handle_enemy_collision = function() {
    for(var i in enemies) {
        ...
        !mage.invincible && !paused) {
            toast.makeToast("Вам нанесён урон от хуйла! Немедленно разберитесь с этим!"); // vvv
            health--;
            update_stats();
...
var load_level = function(id) {
    ...
    if(lvl && (coin_counter >= lvl.cost || lvl.unlocked)) {
        ...
        toast.makeToast("Вам нужно "+lvl.cost
        +" монеток, что попасть в этот уровень "+id+"!");
    } else {
        toast.makeToast("Этот уровень "+id+" находится в разработке");
    }
};
